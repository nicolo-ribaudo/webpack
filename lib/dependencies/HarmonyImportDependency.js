...
/** @typedef {import("../Dependency").DeferImportExperimentOptions} DeferImportExperimentOptions */
...
class HarmonyImportDependency extends ModuleDependency {
	...
	/**
	 * @param {ModuleGraph} moduleGraph module graph
	 * @param {Module} importedModule imported module
	 * @param {DeferImportExperimentOptions} deferCheckMode check mode
	 * @returns {WebpackError | null} errors
	 */
	_getDeferredModuleLinkingErrors(moduleGraph, importedModule, deferCheckMode) {
		if (!this.defer || !deferCheckMode) return null;
		if (deferCheckMode.asyncModule === "error") {
			if (!moduleGraph.isAsync(importedModule)) return null;
			return new HarmonyLinkingError(
				`Module ${this.userRequest} cannot be imported with deferred because it is an async module.`
			);
		}
	}
	 ...
	 * @returns {WebpackError | null} errors
	 */
	_getAssertionLinkingErrors(moduleGraph, importedModule) {
		if (!this.syncAssert) return null;
		if (!moduleGraph.isAsync(importedModule)) return null;
		return new HarmonyLinkingError(
			`Import assertion failed: ${this.userRequest} is not a sync module.`
		);
	}

	// TODO: remove the default value in webpack 6
	/**
	 * @param {ModuleGraph} moduleGraph module graph
	...
	getLinkingErrors(
		...
		const errors = [];
		if (checkMode.useSyncAssertion) {
			const syncError = this._getAssertionLinkingErrors(
				moduleGraph,
				importedModule
			);
			syncError && errors.push(syncError);
		}
		if (checkMode.deferModuleMode) {
			const deferredError = this._getDeferredModuleLinkingErrors(
				moduleGraph,
				importedModule,
				checkMode.deferModuleMode
			);
			deferredError && errors.push(deferredError);
		}
		if (checkMode.strictExportPresence) {
			const exportError = this._getStrictExportPresenceLinkingErrors(
				moduleGraph,
				ids,
				additionalMessage,
				importedModule
			);
			exportError && errors.push(...exportError);
		}
		return errors;
	}
	 ...
	 * @returns {WebpackError[] | undefined} errors
	 */
	_getStrictExportPresenceLinkingErrors(
		moduleGraph,
		ids,
		additionalMessage,
		importedModule
